<template>
  <form @submit.prevent="sendOperation" class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md flex flex-col gap-4">
    ...
    <button type="submit" class="bg-indigo-600 text-white rounded-lg py-2 hover:bg-indigo-700 transition">Envoyer l'opération</button>
...
const sendOperation = async () => {
  try {
    await fetch('http://localhost:5000/api/operation', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ n1: n1.value, n2: n2.value, op: op.value })
    })
    message.value = "Opération envoyée !"
